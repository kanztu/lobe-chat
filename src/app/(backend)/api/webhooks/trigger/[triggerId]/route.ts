...
export async function POST(
  ...
  try {
    ...
    if (config.secret) {
      ...
      const isValid = await verifyHmacSignature(body, signature, config.secret);
      ...
      // Re-create request with parsed JSON
    ...
    // 4. Check rate limit
    const canExecute = await checkRateLimit(trigger.id, trigger.userId);
    if (!canExecute) {
      return NextResponse.json(
 ...
 */
async function checkRateLimit(triggerId: string, userId: string): Promise<boolean> {
